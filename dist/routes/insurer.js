...
const insuranceTypeValues = Object.values(insurancesTypes);
...
insurerRoutes.post('/create', async (c) => {
    try {
        const body = await c.req.json();
        const validatedData = insurerCreateSchema.parse(body);
        const newInsurer = await createInsurer(validatedData);
...
// Protected routes
insurerRoutes.get('/about', authenticate, async (c) => {
    try {
...
insurerRoutes.put('/about', authenticate, async (c) => {
    try {
        const body = await c.req.json();
        const { uid } = c.get('user');
...
insurerRoutes.post('/create/insurance', authenticate, async (c) => {
    try {
        const body = await c.req.json();
        const { uid } = c.get('user');
        const validatedData = insuranceTypeCreateSchema.parse(body);
        const insurance = await createInsurance(uid, validatedData);
...
insurerRoutes.get('/insurances', authenticate, async (c) => {
    try {
        const { uid } = c.get('user');
